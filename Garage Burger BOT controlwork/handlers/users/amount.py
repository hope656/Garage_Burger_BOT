...
async def get_amount(message: types.Message, state: FSMContext):
    ...
    if int(amount) > 0:
        ...
        await message.answer(f"<b>{product_name} Ваш заказ добавлен!</b>\n\n<code>{product_name} x {amount} = {product_price * amount} сум</code>\n\n<i>Вы можете проверить в: КОРЗИНА 🛒</i>", parse_mode="html", reply_markup=cats_markup)
        await ShopState.category.set()
    else:
        await message.answer("Пожалуйста, правильно выберите число ☺️", reply_markup=numbers)